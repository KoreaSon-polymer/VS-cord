...
- NRF uses its current `/page/362`, `/page/364` boards and `data-post_no` detail identifiers. IRIS uses the `ListView` page and resolves notice IDs from its onclick attributes rather than mistaking JavaScript links for real notices. IRIS covers multiple ministries/agencies, including NRF and energy/industrial programs. MSIT uses the business-call board rather than its homepage.
- Main article content and up to three bounded PDF/HWPX/ZIP attachments are read. Legacy binary HWP, scanned PDFs, JavaScript-only download buttons and login-only boards can still require manual review. No OCR or fabricated attachment content.
